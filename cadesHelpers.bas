...
' ==========================================================================================
' REST fast, robust GET/POST. Returns Dictionary.
' ==========================================================================================
' Makes an HTTP request. headers can be Dictionary or 2D array of {name,value}.
' If body is Dictionary/Array and Content-Type not set, auto-serializes to JSON.
' Returns a Dictionary with:
'   Status (Long), StatusText (String), Headers (Dictionary), Body (String),
'   Json (Variant or Empty), Url (String), DurationMs (Long), Success (Boolean)
Public Function HttpRequest(ByVal method As String, _
                            ByVal URL As String, _
                            Optional ByVal body As Variant, _
                            Optional ByVal headers As Variant, _
                            Optional ByVal timeoutMs As Long = 30000, _
                            Optional ByVal allowRedirects As Boolean = True, _
                            Optional ByVal parseJson As Boolean = True, _
                            Optional ByVal username As String = "", _
                            Optional ByVal password As String = "") As Object
    ...
    Dim seg As Long: seg = timeoutMs \ 4
    If seg < 1000 Then seg = 1000
    req.SetTimeouts seg, seg, seg, seg

    req.Option(6) = allowRedirects ' WinHttpRequestOption_EnableRedirects
    ' req.Option(9) = 0 ' SSL error flags (0=none)

    Dim m As String: m = UCase$(Trim$(method))
    If m = "" Then m = "GET"

    ' Prepare body / default headers
    Dim hasBody As Boolean: hasBody = Not IsMissing(body)
    Dim ContentType As String: ContentType = ""

    Dim hdr As Object: Set hdr = NewDictionary(True)
    If Not IsMissing(headers) Then
        If IsObject(headers) Then
            Dim k As Variant
            For Each k In headers.keys
                hdr(CStr(k)) = CStr(headers(k))
            Next
        ElseIf IsArray(headers) Then
            Dim i As Long
            For i = LBound(headers) To UBound(headers)
                If IsArray(headers(i)) Then
                    hdr(CStr(headers(i)(0))) = CStr(headers(i)(1))
                End If
            Next
        End If
    End If

    If hdr.exists("Content-Type") Then ContentType = hdr("Content-Type")
    ...
    If hasBody Then
        If IsObject(body) Or IsArray(body) Then
            ...
            End If
            If InStr(1, ContentType, "json", vbTextCompare) > 0 Then
                bodyStr = JSON_Stringify(body, False)
            Else
                bodyStr = AsString(body)
            End If
        Else
            bodyStr = AsString(body)
        End If
    ...
    If Len(username) > 0 Or Len(password) > 0 Then
        req.Open m, URL, False, username, password
    Else
        req.Open m, URL, False
    End If
    ' Default Accept if not provided
    If Not hdr.exists("Accept") Then hdr("Accept") = "application/json, */*"
    Dim hk As Variant
    For Each hk In hdr.keys
        req.setRequestHeader CStr(hk), CStr(hdr(hk))
    Next

    On Error GoTo EH
    If hasBody And (m = "POST" Or m = "PUT" Or m = "PATCH") Then
        req.Send bodyStr
    Else
        req.Send
    End If
    ...
    ' Build response
    Dim resp As Object: Set resp = NewDictionary(True)
    resp("Status") = CLng(req.Status)
    resp("StatusText") = CStr(req.statusText)
    resp("Body") = CStr(req.responseText)
    resp("Url") = URL
    resp("DurationMs") = CLng((Timer - t0) * 1000)
    resp("Success") = (resp("Status") >= 200 And resp("Status") < 300)

    ' Parse headers
    Dim rawHeaders As String: rawHeaders = CStr(req.GetAllResponseHeaders)
    Dim dh As Object: Set dh = NewDictionary(False)
    Dim lines() As String: lines = Split(rawHeaders, vbCrLf)
    Dim line As Variant, p As Long
    For Each line In lines
        If Len(line) > 0 Then
            p = InStr(1, line, ":", vbBinaryCompare)
            If p > 0 Then dh(Trim$(Left$(line, p - 1))) = Trim$(mid$(line, p + 1))
        End If
    Next
    resp("Headers") = dh
    ...
    If parseJson Then
        ...
        parsed = JSON_ToObject(resp("Body"))
...
EH:
    Dim er As Object: Set er = NewDictionary(True)
    er("Status") = 0
    er("StatusText") = "ERROR: " & Err.Description
    er("Body") = ""
    er("Headers") = NewDictionary(True)
    er("Url") = URL
    er("DurationMs") = CLng((Timer - t0) * 1000)
    er("Success") = False
    Set HttpRequest = er
End Function
